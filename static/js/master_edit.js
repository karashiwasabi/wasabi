...
function createMasterRowHTML(master = {}) {
    ...
    const topRowFields = [
        { key: 'productCode', ph: '製品コード(JAN)', readonly: !isNew }, { key: 'yjCode', ph: 'YJコード' },
        { key: 'productName', ph: '製品名' }, { key: 'kanaName', ph: 'カナ名' }, { key: 'makerName', ph: 'メーカー名' },
        { key: 'usageClassification', ph: '剤型(JC013)' }, { key: 'nhiPrice', ph: '薬価', type: 'number', step: '0.0001', value: master.nhiPrice || 0 },
    ];
    const flags = [
        ...
        { key: 'flagStimulant', ph: '覚', opts: [0, 1] }, { key: 'flagStimulantRaw', ph: '覚原', opts: [0, 1] },
    ];
    // ▼▼▼ [修正点] packageSpec を項目から削除し、packageFormに統一 ▼▼▼
    const bottomRowFields = [
        { key: 'packageForm', ph: '包装(JC037)'}, { key: 'yjUnitName', ph: 'YJ単位' },
        { key: 'yjPackUnitQty', ph: 'YJ包装数量', type: 'number', value: master.yjPackUnitQty || 0 }, 
        { key: 'janPackInnerQty', ph: '内包装数量', type: 'number', value: master.janPackInnerQty || 0 },
        { key: 'janUnitCode', ph: 'JAN単位', type: 'select', value: master.janUnitCode || 0}, 
        { key: 'janPackUnitQty', ph: 'JAN包装数量', type: 'number', value: master.janPackUnitQty || 0 },
    ];

    let topRowCellsHTML = topRowFields.map(f => `<td><input type="${f.type || 'text'}" name="${f.key}" value="${f.value || master[f.key] || ''}" placeholder="${f.ph}" ${f.readonly ? 'readonly' : ''} ${f.step ? `step="${f.step}"` : ''}></td>`).join('');
    ...
    topRowCellsHTML += `<td><button class="save-master-btn btn">保存</button></td>`;
    
    let bottomRowCellsHTML = bottomRowFields.map(f => {
        if (f.type === 'select') {
            let options = `<option value="0">YJ単位と同じ</option>`;
    ...
    bottomRowCellsHTML += `<td class="formatted-spec-cell" colspan="7"></td><td><button class="quote-jcshms-btn btn">引用</button></td>`;
...
async function loadAndRenderMasters() {
    tableContainer.innerHTML = `<table><tbody><tr><td>読み込み中...</td></tr></tbody></table>`;
    try {
        ...
        const tableContent = masters.map(createMasterRowHTML).join('');
        tableContainer.innerHTML = `<table class="data-table">${tableHeader}${tableContent}</table>`;
...
export async function initMasterEdit() {
    ...
    tableContainer.addEventListener('click', async (e) => {
        ...
        if (target.classList.contains('save-master-btn')) {
            ...
            // ▼▼▼ [修正点] packageFormの値をpackageSpecにもコピーして、データを確実に送信 ▼▼▼
